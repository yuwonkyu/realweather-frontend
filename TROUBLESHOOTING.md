...
- **일관성**: 픽셀 단위까지 정확히 일치 (`w-64` = `ml-64`)
- **타입 안전성**: 전역 타입 선언으로 빌드 오류 해결
- **환경 분리**: 브라우저/Node.js 환경 체크로 안정적인 배포

### ⚠️ 개선 사항

- **공식 문서 확인**: 메이저 버전 업그레이드 시 Breaking Changes 숙지
- **디버깅 순서**: CSS → 브라우저 환경 → 타입 → 컴포넌트 로직
- **근본 원인 파악**: 시각적 피드백이 없을 때 CSS/빌드 설정부터 확인
